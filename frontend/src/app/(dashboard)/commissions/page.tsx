...
import { TrendingUp, CheckSquare, Trash2 } from 'lucide-react';
...
export default function CommissionsPage() {
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => apiHelper.delete(`/commissions/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['commissions'] }); qc.invalidateQueries({ queryKey: ['commissions-report'] }); toast.success('Commission supprimée'); },
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {!isLoading && commissions.length === 0 && (
                <tr><td colSpan={12} className="text-center py-8 text-gray-400 text-sm">Aucune commission</td></tr>
              )}
              {commissions.map((c: any) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="table-cell">
                    <button
                      onClick={() => { if (window.confirm('Supprimer cette commission ? Cette action est irréversible.')) deleteMut.mutate(c.id); }}
