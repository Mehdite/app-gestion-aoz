...
/* ═══════════════════════════════════════════════════════════════
   FORMULAIRE DE SAISIE — PRODUCTION AXA UNIQUEMENT
   Volontairement indépendant de CAT et Cover EDGE : toute évolution
   ici (types, sous-catégories, champs) ne doit toucher qu'AXA.
   ═══════════════════════════════════════════════════════════════ */
...
const COMPANY_CODE = 'AXA';
const RETURN_PATH  = '/contrats';
...
/** Précision exigée pour Automobile et Moto — propre à AXA */
...
export function SaisirProductionAxa() {
          ...
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" className="input" placeholder="Ex: AXA-2024-000123" {...register('contractNumber')} />
            ...
            {/* Précision — Automobile et Moto seulement */}
            {sousCategories && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label">Précision {TYPES.find((t) => t.value === typeChoisi)?.label} *</label>
                  <select className="input" {...register('sousCategorie')}>
                    <option value="">Sélectionner...</option>
                    {sousCategories.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                  {errors.sousCategorie && <p className="text-xs text-red-500 mt-1">{errors.sousCategorie.message}</p>}
                </div>
              </div>
            )}
